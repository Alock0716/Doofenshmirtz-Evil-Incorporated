...
app.post("/api/v1/plaid/create-link-token", requireAuth, async (req, res) => {
  try {
    ...
    return res.json({
      linkToken: "replace-with-real-link-token",
...
app.post("/api/v1/plaid/exchange-public-token", requireAuth, async (req, res) => {
  try {
    ...
    // TODO: call Plaid itemPublicTokenExchange here
    const accessTokenValue = "replace-with-access-token";
    const itemIdValue = "replace-with-item-id";
...
app.delete("/api/v1/plaid/items/:itemId", requireAuth, async (req, res) => {
  try {
    ...
    const resultValue = await runQuery(
      `
